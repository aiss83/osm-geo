...
Реализовано в ветке `gol`:

- **Этапы 1–2** — `src/gol.rs` (конвертация PBF→GOL), `src/source.rs`
  (`FeatureSource`, `SourceKind`, `detect_source`), флаг `--source`.
- **Этап 3** — чтение GOL через CLI (`gol query`, два GOQL-запроса + конкатенация
  PBF) в `src/gol.rs`.
- **Этап 4** — сквозная сверка на Калининграде и Central Fed District: сырое
  извлечение идентично PBF.
- **Этап 5** — **FFI-бэкенд реализован** (`gol-ffi` feature, C ABI поверх
  libgeodesk: `gol-ffi/gol_ffi.{h,cpp}`, `src/gol_ffi.rs`, `build.rs`) — чтение
  без промежуточного PBF, в ~11× быстрее.

Дополнительно оптимизирован `infer_missing_cities` (пространственная сетка,
~108× быстрее). Подробности и замеры — в `docs/GOL_optimization.md`.
