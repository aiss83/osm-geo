...
**`.osmg`** (версия формата `2`).

Файл `.osmg` самодостаточен и не требует SQLite/FTS5/R*Tree на устройстве:
...
Для транспортировки файл сжимается Zstandard: `.osmg.zst`.
...
В `.osmg` встроен инвертированный индекс для поиска по улице/имени без построения
...
иначе запрос не совпадёт со стеммированными токенами в `.osmg`.
...
- `{страна}-{код страны}-{числовой код страны}.osmg` — несжатая база (напр. `russia-ru-643.osmg`);
- `{страна}-{код страны}-{числовой код страны}.osmg.zst` — сжатая Zstandard (уровень 3) версия для передачи;
- `{страна}-{код страны}-{числовой код страны}.metadata.json` — метаданные.
...
`sha256` — хеш **несжатого** `.osmg` файла.
