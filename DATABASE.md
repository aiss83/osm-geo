...
На этапе сборки все названия улиц и городов проходят через:

1. **Коррекцию опечаток** (SymSpell) — по частотному словарю русского языка (`ru_full.txt`, ~1.4M слов, 28 МБ). Словарь включён в репозиторий; при его отсутствии скачивается автоматически.
2. **Нормализацию регистра** — «улица Ленина» (префиксный тип улицы со строчной буквы, название с заглавной), «Красная площадь» (регистр предложения).

Коррекция устраняет типичные опечатки в OSM-данных (напр. «Масква» → «Москва») и уменьшает задвоение записей из-за разного регистра.
